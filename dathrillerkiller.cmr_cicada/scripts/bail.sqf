...
format ["if (iscop) then {[player,'geld',%2] call storage_add; systemChat  format [localize ""STRS_bail_addbail"", %2, ""%3""];}; titletext [format[localize ""STRS_bail_payed"",%3, %1], ""plain""]; _boun = (%4 getvariable 'cdb_bounty'); _boun = _boun - %1; if(_boun < 0)then{_boun = 0};
